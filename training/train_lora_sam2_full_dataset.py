#!/usr/bin/env python3
"""
Train SAM2 Large model with LoRA adapters on the full Severstal dataset.
...
Training data: datasets/Data/splits/train_split (full training set)
...
class LoRALayer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        lora_B = self.lora_B.to(device)
                        ...
                        # Debug: print tensor shapes (only once per layer) - disabled for cleaner output
                # if not hasattr(self, '_debug_printed'):
                #     print(f"LoRA forward: input shape={x.shape}, lora_A shape={lora_A.shape}, lora_B shape={lora_B.shape}")
                #     print(f"  Layer dimensions: in_features={self.in_features}, out_features={self.out_features}")
...
def prepare_full_dataset():
    """
    Prepare full dataset using train_split for training and val_split for validation.
    """
    # Use relative paths (script must be run from project root)
    ...
    with torch.no_grad():
        for entry in val_data:
            try:
                ...
                # Debug shapes - disabled for cleaner output
                # print(f"Validation - gt_mask shape: {gt_mask.shape}, pred_binary shape: {pred_binary.shape}")
                ...
                if gt_mask.shape != pred_binary.shape:
                    ...
                    # print(f"Validation - Resized pred_binary to: {pred_binary.shape}")
...
def train_sam2_lora_full_dataset(learning_rate=1e-3, weight_decay=0.01, steps=10000, 
                                batch_size=1, lora_rank=8, lora_alpha=16, lora_dropout=0.05):
    """
    Train SAM2 Large model with LoRA adapters on the full dataset.
    ...
    print(f"\n{'='*80}")
    print(f"Training SAM2 LARGE with LoRA on Full Dataset")
    print(f"{'='*80}")
    ...
    # Configuration - use relative paths (script must be run from project root)
    ...
    # Prepare dataset
    train_data, val_data = prepare_full_dataset()
    ...
    model_name = f"sam2_large_lora_r{lora_rank}_a{lora_alpha}_lr{str(learning_rate).replace('e-', 'e')}_{timestamp}"
    ckpt_dir = os.path.join(project_root, f"new_src/training/training_results/sam2_lora/{model_name}")
    os.makedirs(ckpt_dir, exist_ok=True)
    
    # Metrics logger
    logger = MetricsLogger(os.path.join(project_root, f"logs/training_metrics_lora_{timestamp}.csv"))
    ...
    for step in range(1, steps + 1):
        try:
            with torch.amp.autocast('cuda'):
                ...
                # print(f"Debug - gt_mask shape: {gt_mask.shape}, low_res_masks shape: {low_res_masks.shape}")
    ...
    summary = {
        ...
        "note": "LoRA training with attention projections only. Early stopping with patience=7."
    }
...
def main():
    parser = argparse.ArgumentParser(description="Train SAM2 Large with LoRA on full dataset")
    ...
    # Train with LoRA
    try:
        result = train_sam2_lora_full_dataset(
