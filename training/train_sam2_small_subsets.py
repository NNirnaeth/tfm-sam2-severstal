#!/usr/bin/env python3
"""
Train SAM2 Small model on different subset sizes from datasets/Data/splits/subsets
        ...
        for entry in val_data:
            try:
                ...
                if gt_mask.shape != pred_binary.shape:
                    print(f"  Shape mismatch - gt: {gt_mask.shape}, pred: {pred_binary.shape}")
...
def train_sam2_small_on_subset(subset_size, learning_rate, weight_decay, steps, batch_size=1):
    """
    Train SAM2 small model on a specific subset size.
    ...
    print(f"\n{'='*80}")
    print(f"Training SAM2 SMALL on Subset {subset_size}")
    print(f"{'='*80}")
...
def main():
    parser = argparse.ArgumentParser(description="Train SAM2 Small on different subset sizes")
    ...
    print("SAM2 Small Subsets Training Script")
    print("=" * 50)
    print(f"Training SAM2 small on subsets: {args.subset_sizes}")
    ...
    for subset_size in args.subset_sizes:
        ...
        try:
            result = train_sam2_small_on_subset(
                subset_size=subset_size,
