#!/usr/bin/env python3
"""
Train DSC U-Net (Deep Supervision + Dense Skip Connections) on full Severstal dataset
...
Loss: DSC (Dice) directly as loss function

This script uses modular components:
- experiments/model_unet.py for model creation (modified for DSC)
- experiments/prepare_data_unet.py for data loading

TFM Requirements:
- AdamW with weight_decay=1e-4
- Compare lr = 1e-3 vs 1e-4
- DSC loss directly (no BCE combination)
...
def dsc_loss(pred, target, smooth=1e-6):
    """DSC (Dice) loss for binary segmentation - used directly as loss function"""
    ...
    # Resize target to match pred if sizes are different
...
def train_epoch(model, dataloader, criterion, optimizer, device, scaler):
    ...
    for batch_idx, (images, masks) in enumerate(tqdm(dataloader, desc="Training")):
        ...
        with torch.amp.autocast('cuda'):
            ...
            # DSC UNet outputs have different spatial dimensions, need to resize to match masks
...
def main():
    parser = argparse.ArgumentParser(description='Train DSC UNet on Severstal dataset')
    parser.add_argument('--train_path', type=str, 
                       default='datasets/Data/splits/train_split',
                       help='Path to training images')
    parser.add_argument('--val_path', type=str, 
                       default='datasets/Data/splits/val_split',
                       help='Path to validation images')

    parser.add_argument('--save_dir', type=str, 
                       default='new_src/training/training_results/unet_models',
    ...
    if args.use_wandb:
        wandb.init(
            project="dsc-unet-severstal",
            name=f"dsc_unet_{args.encoder}_full_dataset_seed{args.seed}",
            config=vars(args)
        )
    ...
    # Create model (DSC UNet with dense skip connections)
    print("Creating DSC UNet model...")
    model = create_unet_model(
        encoder_name=args.encoder,
        unet_plus_plus=True,  # Use dense skip connections
        in_channels=3,
        num_classes=1
    )
    ...
    # Loss function: DSC directly (no BCE combination)
    criterion = dsc_loss
    ...
    print(f"  Model: DSC UNet++ with {args.encoder}")
    ...
    print(f"  Loss function: DSC (Dice) directly")
