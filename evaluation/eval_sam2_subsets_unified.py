#!/usr/bin/env python3
"""
Evaluate SAM2 Small and Large models trained on different subset sizes (100, 200, 500, 1000, 2000, 4000)
using the best checkpoints from training.

This script evaluates all fine-tuned SAM2 models on the complete test split
with three evaluation modes:
1. SAM2-AutoPrompt (no GT, no external detector)
2. 1-3 GT points (centroid + max distance + negative)
3. 30 GT points (for comparison with previous results)

Comprehensive metrics and visualizations are included.
"""
...
def evaluate_single_model(model_info, test_images, test_ann_dir, results_dir, evaluation_mode="auto_prompt", 
                         num_gt_points=3, num_viz_samples=5):
    """Evaluate a single SAM2 model on the test set"""
    model_path, model_name, subset_size, lr, model_size = model_info
    ...
    model_results_dir = os.path.join(results_dir, f"{model_size}_subset_{subset_size}{mode_suffix}")
    ...
    try:
        print(f"Loading fine-tuned SAM2 {model_size} model...")
        ...
        # Use appropriate config file based on model size
        if model_size == "small":
    ...
    if evaluation_mode == "auto_prompt":
        print(f"Prompt settings: grid_spacing={GRID_SPACING}, box_scales={BOX_SCALES}")
    print(f"Confidence threshold: {CONFIDENCE_THRESHOLD}, NMS IoU threshold: {NMS_IOU_THRESHOLD}")
    
    # Evaluation loop with torch.no_grad() for stability
    with torch.no_grad():
        for idx, img_file in enumerate(tqdm(test_images, desc=f"Evaluating {model_name}")):
            ...
            if evaluation_mode == "auto_prompt":
                ...
                total_prompts = len(auto_prompts['points']) + len(auto_prompts['boxes'])
                prompts_per_image.append(total_prompts)
            ...
            try:
                ...
                if evaluation_mode == "auto_prompt":
                    pred_mask, num_final_masks = predict_with_auto_prompts(sam2_predictor, image, auto_prompts)
                else:
                    pred_mask, num_final_masks = predict_with_gt_points(sam2_predictor, image, gt_points, gt_labels)
            ...
            # Compute IoU for this sample with proper handling of empty masks
            intersection = np.logical_and(pred_mask, gt_mask).sum()
            union = np.logical_or(pred_mask, gt_mask).sum()
            
            if union == 0:
                # Both GT and prediction are empty - define IoU = 1
                iou = 1.0
            else:
    ...
    # Safety check: ensure we processed at least some images
    ...
    batch_results['mean_prompts_per_image'] = float(np.mean(prompts_per_image))
    batch_results['std_prompts_per_image'] = float(np.std(prompts_per_image))
    batch_results['total_prompts_used'] = int(np.sum(prompts_per_image))
    batch_results['no_prediction_rate'] = float(no_prediction_count / len(all_predictions))
    ...
    model_info_csv = {
        'model': f'sam2_{model_size}',
        'variant': f'subset_{subset_size}',
        'prompt_type': evaluation_mode,
    ...
    additional_info = {
        ...
        'grid_spacing': GRID_SPACING if evaluation_mode == "auto_prompt" else None,
        'box_scales': BOX_SCALES if evaluation_mode == "auto_prompt" else None,
    ...
    # Save results to CSV
    experiment_name = f"eval_sam2_{model_size}_subset_{subset_size}_{evaluation_mode}_{datetime.now().strftime('%Y%m%d_%H%M')}"
    csv_path = metrics_calculator.save_results_to_csv(
        batch_results, experiment_name, model_info_csv, additional_info
    )
    
    # Print comprehensive summary
    metrics_calculator.print_metrics_summary(batch_results, f"SAM2 {model_size.title()} (Subset {subset_size})")
    
    # Print timing and prompt information
    print(f"\n{'='*60}")
    print("TIMING & PROMPT INFORMATION")
    print(f"{'='*60}")
    ...
    print(f"Mean prompts per image: {batch_results['mean_prompts_per_image']:.1f}")
    print(f"Total prompts used: {batch_results['total_prompts_used']}")
    print(f"No prediction rate: {batch_results['no_prediction_rate']:.3f}")
...
def find_subset_models(model_size="both"):
    ...
    for item in os.listdir(models_dir):
        ...
        if os.path.isdir(item_path) and 'subset_' in item:
            if model_size == "both":
                if 'sam2_small' in item or 'sam2_large' in item:
                    pass  # Include both
                else:
                    continue
            elif model_size == "small" and 'sam2_small' not in item:
                continue
            elif model_size == "large" and 'sam2_large' not in item:
                continue
            ...
            for i, part in enumerate(parts):
                if part == 'subset' and i + 1 < len(parts):
                    ...
                    if i + 1 < len(parts):
                        current_model_size = parts[i + 1]
            
            if subset_size is not None and lr is not None and current_model_size is not None:
                # Find best checkpoint
                best_ckpt = find_best_checkpoint(item_path)
                if best_ckpt:
                    model_name = f"SAM2 {current_model_size.title()} - Subset {subset_size} - LR {lr}"
    ...
    # Sort by model size, then subset size
    subset_models.sort(key=lambda x: (x['model_size'], x['subset_size']))
    
    return subset_models

def main():
    parser = argparse.ArgumentParser(description='Evaluate SAM2 Small and Large models on different subset sizes')
    ...
    parser.add_argument('--results_dir', type=str, 
                       default='/home/ptp/sam2/new_src/evaluation/evaluation_results/sam2_subsets_unified',
    ...
    if args.test_only:
        ...
        subset_models = find_subset_models(args.model_size)
        if subset_models:
            ...
            test_result = evaluate_single_model(
                (test_model['checkpoint_path'], test_model['model_name'], 
                 test_model['subset_size'], test_model['learning_rate'], test_model['model_size']),
        ...
        try:
            result = evaluate_single_model(
                (model_info['checkpoint_path'], model_info['model_name'], 
                 model_info['subset_size'], model_info['learning_rate'], model_info['model_size']),
                test_images,
                test_ann_dir,
                args.results_dir,
                evaluation_mode=args.evaluation_mode,
            ...
            if result:
                key = f"{model_info['model_size']}_{model_info['subset_size']}"
                all_results[key] = {
                    'success': True,
                    'metrics': result,
            ...
            print(f"Error evaluating {model_info['model_size']} subset {model_info['subset_size']}: {e}")
            key = f"{model_info['model_size']}_{model_info['subset_size']}"
            all_results[key] = {
                'success': False,
                'error': str(e)
    ...
    if successful_results:
        ...
        print(f"{'='*80}")
        print(f"{'Model':<15} {'Subset':<8} {'mIoU':<8} {'Dice':<8} {'IoU@50':<8} {'IoU@75':<8} {'Time/img':<10}")
        print(f"{'='*80}")
        
        for key in sorted(successful_results.keys()):
            result = successful_results[key]['metrics']
            model_info = successful_results[key]['model_info']
            print(f"{model_info['model_size']:<15} {model_info['subset_size']:<8} {result.get('mean_iou', 0):<8.4f} "
    ...
    summary_file = os.path.join(args.results_dir, f"evaluation_summary_{args.evaluation_mode}_{datetime.now().strftime('%Y%m%d_%H%M')}.json")
