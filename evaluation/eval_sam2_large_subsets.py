#!/usr/bin/env python3
"""
Evaluate SAM2 Large models trained on different subset sizes (100, 200, 500, 1000, 2000, 4000)
using the best checkpoints from training.

This script evaluates all fine-tuned SAM2 Large models on the complete test split
...
def evaluate_single_model(model_info, test_images, test_ann_dir, results_dir, num_viz_samples=5):
    """Evaluate a single SAM2 Large model on the test set"""
    ...
    try:
        ...
        # Use relative path from sam2base directory - SAM2 Large config
    ...
    abs_model_results_dir = os.path.abspath(model_results_dir)
    metrics_calculator = SegmentationMetrics(save_dir=abs_model_results_dir)
    ...
    with torch.no_grad():
        for idx, img_file in enumerate(tqdm(test_images, desc=f"Evaluating {model_name}")):
            ...
            point_calls = (len(auto_prompts['points']) + 3) // 4  # Ceiling division for batch size 4
    ...
    # Save results to CSV
    experiment_name = f"eval_sam2_large_subset_{subset_size}_{datetime.now().strftime('%Y%m%d_%H%M')}"
    csv_path = metrics_calculator.save_results_to_csv(
        batch_results, experiment_name, model_info_csv, additional_info
    )
    
    # Print comprehensive summary
    metrics_calculator.print_metrics_summary(batch_results, f"SAM2 Large (Subset {subset_size})")
...
def find_subset_models():
    ...
    # Look for subset directories - ONLY Large models
    for item in os.listdir(models_dir):
        item_path = os.path.join(models_dir, item)
        
        # Only process Large models
        if os.path.isdir(item_path) and 'subset_' in item and 'sam2_large' in item:
            # Extract subset size from directory name
            # Example: sam2_large_subset_100_lr1e4_20241201_1200
            parts = item.split('_')
            subset_size = None
            lr = None
            ...
            if subset_size is not None and lr is not None:
                ...
                if best_ckpt:
                    model_name = f"SAM2 Large - Subset {subset_size} - LR {lr}"
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate SAM2 Large models on different subset sizes')
