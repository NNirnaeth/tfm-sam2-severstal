...
MIN_PIOU = 0.05  # Minimum point IoU threshold (0.05-0.10 range)
MASK_THRESHOLD = 0.45  # Mask binarization threshold (0.40-0.50 range)
CONFIDENCE_THRESHOLD = 0.3  # Fallback confidence threshold
NMS_IOU_THRESHOLD = 0.7  # NMS threshold for duplicate removal
TOP_K_FILTER = 200

# Postprocessing parameters
MIN_AREA = 12  # Minimum area for mask filtering (8-16 px range)
OPENING_KERNEL_SIZE = 3  # Opening kernel size for postprocessing
...
def postprocess_mask(mask, min_area=MIN_AREA, opening_kernel_size=OPENING_KERNEL_SIZE):
    """Apply soft postprocessing to mask"""
    from scipy.ndimage import binary_opening, label
    
    # Convert to binary
    binary_mask = (mask > MASK_THRESHOLD).astype(bool)
    ...
    if min_area > 0:
        ...
        for i in range(1, num_features + 1):
            if np.sum(labeled == i) < min_area:
                binary_mask[labeled == i] = False
    
    # Apply opening to smooth the mask
    if opening_kernel_size > 0:
        kernel = np.ones((opening_kernel_size, opening_kernel_size), dtype=bool)
        binary_mask = binary_opening(binary_mask, structure=kernel)
    
    return binary_mask.astype(np.float32)

def aggregate_masks(masks, scores, use_nms=True, use_union=True):
    """Aggregate multiple masks with optional NMS and union"""
    if not masks:
        return np.zeros((1024, 1024), dtype=bool), 0
    
    # Apply NMS if requested
    if use_nms and len(masks) > 1:
        masks, scores = apply_nms(masks, scores)
    ...
    if use_union:
        # Union of all masks
        aggregated = np.zeros_like(masks[0], dtype=bool)
        for mask in masks:
            aggregated = np.logical_or(aggregated, mask)
        return aggregated, len(masks)
    else:
        # Return best mask
        best_idx = np.argmax(scores)
        return masks[best_idx], 1

def predict_with_auto_prompts(predictor, image, auto_prompts, min_piou=MIN_PIOU, 
                             mask_threshold=MASK_THRESHOLD, fallback_threshold=CONFIDENCE_THRESHOLD,
                             point_batch_size=POINT_BATCH_SIZE, box_batch_size=BOX_BATCH_SIZE):
    """Predict using automatic prompts with improved confidence handling and fallback"""
    height, width = image.shape[:2]
    ...
    if len(auto_prompts['points']) > 0:
        try:
            ...
            for i in range(0, len(points), point_batch_size):
                ...
                # Apply min_pIoU threshold
                for j, score in enumerate(point_scores):
                    if score >= min_piou:
                        mask = point_masks[j].astype(np.float32)
        ...
        try:
            ...
            for box in boxes:
                ...
                for j, score in enumerate(box_scores):
                    if score >= min_piou:
                        mask = box_masks[j].astype(np.float32)
                        all_masks.append(mask)
                        all_scores.append(score)
    ...
    # Fallback: if no masks pass min_pIoU, take the best ones
    if not all_masks:
        ...
        if len(auto_prompts['points']) > 0:
            try:
                ...
                for i in range(0, len(points), point_batch_size):
                    ...
                    if point_scores[best_idx] >= fallback_threshold:
                        mask = point_masks[best_idx].astype(np.float32)
                        all_masks.append(mask)
                        all_scores.append(point_scores[best_idx])
                        
            except Exception as e:
                print(f"Error with fallback point prompts: {e}")
    ...
    # Aggregate masks with NMS and union
    aggregated_mask, num_masks = aggregate_masks(all_masks, all_scores, use_nms=True, use_union=True)
    
    # Apply postprocessing
    processed_mask = postprocess_mask(aggregated_mask, min_area=MIN_AREA, opening_kernel_size=OPENING_KERNEL_SIZE)
    
    return processed_mask.astype(bool), num_masks

def predict_with_gt_points(predictor, image, gt_points, gt_labels, min_piou=MIN_PIOU, 
                          mask_threshold=MASK_THRESHOLD, fallback_threshold=CONFIDENCE_THRESHOLD):
    """Predict using GT points with improved confidence handling and fallback"""
    height, width = image.shape[:2]
    ...
    try:
        ...
        # Apply min_pIoU threshold first
        valid_masks = []
        valid_scores = []
        
        for i, score in enumerate(point_scores):
            if score >= min_piou:
                valid_masks.append(point_masks[i])
                valid_scores.append(score)
        
        # Fallback: if no masks pass min_pIoU, take the best one
        if not valid_masks:
            best_idx = np.argmax(point_scores)
            if point_scores[best_idx] >= fallback_threshold:
                valid_masks = [point_masks[best_idx]]
                valid_scores = [point_scores[best_idx]]
            else:
                return np.zeros((height, width), dtype=bool), 0
        
        # Aggregate masks with NMS and union
        aggregated_mask, num_masks = aggregate_masks(valid_masks, valid_scores, use_nms=True, use_union=True)
...
def load_image_and_mask(img_path, ann_path):
    ...
    mask_resized = cv2.resize(mask_uint8, (image_resized.shape[1], image_resized.shape[0]), interpolation=cv2.INTER_NEAREST)
...
def evaluate_lora_sam2(predictor, test_data, threshold=0.5, save_examples=False, output_dir=None, 
                      ...
                      min_piou=MIN_PIOU, mask_threshold=MASK_THRESHOLD, fallback_threshold=CONFIDENCE_THRESHOLD,
                      min_area=MIN_AREA, opening_kernel=OPENING_KERNEL_SIZE):
    ...
    with torch.no_grad():
        for idx, (img_path, ann_path) in enumerate(tqdm(test_data, desc="Evaluating")):
            try:
                ...
                if evaluation_mode == "auto_prompt":
                    pred_mask, num_masks = predict_with_auto_prompts(
                        predictor, image, auto_prompts,
                        min_piou=min_piou,
                        mask_threshold=mask_threshold,
                        fallback_threshold=fallback_threshold
                    )
                else:
                    pred_mask, num_masks = predict_with_gt_points(
                        predictor, image, gt_points, gt_labels,
                        min_piou=min_piou,
                        mask_threshold=mask_threshold,
                        fallback_threshold=fallback_threshold
                    )
...
def save_results_csv(results, model_info, output_dir="new_src/evaluation/evaluation_results/sam2_lora", 
    ...
    elif 'SAM2-Base+LoRA' in model_label:
        subset = model_info.get('subset', 'unknown')
        variant = f"base_lora_r{model_info['rank']}_a{model_info['alpha']}_lr{model_info['lr']}_subset{subset}"
        exp_prefix = f"exp_base_lora_sam2_subset{subset}"
    elif 'SAM2-FT+LoRA' in model_label:
        subset = model_info.get('subset', 'unknown')
        variant = f"ft_lora_r{model_info['rank']}_a{model_info['alpha']}_lr{model_info['lr']}_subset{subset}"
        exp_prefix = f"exp_ft_lora_sam2_subset{subset}"
    else:
        # Default case
        variant = "unknown"
        exp_prefix = "exp_unknown"
...
def main():
    ...
    parser.add_argument('--min_piou', type=float, default=MIN_PIOU,
                       help='Minimum point IoU threshold (0.05-0.10 range)')
    parser.add_argument('--mask_threshold', type=float, default=MASK_THRESHOLD,
                       help='Mask binarization threshold (0.40-0.50 range)')
    parser.add_argument('--fallback_threshold', type=float, default=CONFIDENCE_THRESHOLD,
                       help='Fallback confidence threshold when min_piou fails')
    parser.add_argument('--min_area', type=int, default=MIN_AREA,
                       help='Minimum area for mask filtering (8-16 px range)')
    parser.add_argument('--opening_kernel', type=int, default=OPENING_KERNEL_SIZE,
                       help='Opening kernel size for postprocessing')
