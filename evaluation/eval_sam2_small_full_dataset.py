...
def evaluate_sam2_small_full_dataset(checkpoint_path, results_dir, num_viz_samples=10, evaluation_mode="auto_prompt", num_gt_points=3):
    ...
    # Set seed for reproducibility (redundant but ensures 100% determinism)
    set_seed(42)
    ...
    # Initialize metrics - use absolute path to avoid path resolution issues
    abs_results_dir = os.path.abspath(results_dir)
    metrics_calculator = SegmentationMetrics(save_dir=abs_results_dir)
    ...
    if evaluation_mode == "auto_prompt":
        ...
        print(f"Point batch size: {POINT_BATCH_SIZE}, Box batch size: {BOX_BATCH_SIZE}")
    ...
    with torch.no_grad():
        for idx, img_file in enumerate(tqdm(test_images, desc="Evaluating")):
            ...
            if evaluation_mode == "auto_prompt":
                ...
                # Calculate number of predict calls (proxy for computational cost)
                point_calls = (len(auto_prompts['points']) + POINT_BATCH_SIZE - 1) // POINT_BATCH_SIZE  # Ceiling division
                box_calls = len(auto_prompts['boxes'])  # One call per box
                total_predict_calls = point_calls + box_calls
                predict_calls_per_image.append(total_predict_calls)
                ...
                auto_prompts = None
                prompts_per_image.append(len(gt_points))
                predict_calls_per_image.append(1)  # Single call for GT points
            elif evaluation_mode == "30_gt_points":
            ...
            try:
                ...
                # Track final masks count (instances detected by SAM2)
                final_masks_counts.append(num_final_masks)
                ...
                pred_mask = np.zeros_like(gt_mask)
                no_prediction_count += 1
                final_masks_counts.append(0)
            
            inference_time = time.time() - start_time
            inference_times.append(inference_time)
            
            # Store for metrics (ensure uint8 for SegmentationMetrics compatibility)
            all_predictions.append(pred_mask.astype(np.uint8))
            all_ground_truths.append(gt_mask.astype(np.uint8))
    ...
    batch_results['mean_predict_calls_per_image'] = float(np.mean(predict_calls_per_image))
    batch_results['total_predict_calls'] = int(np.sum(predict_calls_per_image))
    
    # Add final masks metrics (instances detected by SAM2)
    batch_results['mean_final_masks_per_image'] = float(np.mean(final_masks_counts))
    batch_results['std_final_masks_per_image'] = float(np.std(final_masks_counts))
    ...
    # Add auto-prompt specific parameters only for auto_prompt mode
    if evaluation_mode == "auto_prompt":
    ...
    print(f"Mean predict calls per image: {batch_results['mean_predict_calls_per_image']:.1f}")
    print(f"Total predict calls: {batch_results['total_predict_calls']}")
    print(f"Mean final masks per image: {batch_results['mean_final_masks_per_image']:.1f}")
    print(f"Std final masks per image: {batch_results['std_final_masks_per_image']:.1f}")
...
def test_script_functionality(checkpoint_path, num_test_images=5, evaluation_mode="auto_prompt", num_gt_points=3):
    ...
    try:
        ...
        success_count = 0
        with torch.no_grad():  # Add torch.no_grad for consistency
            for img_file in test_images:
                ...
                # Test auto-prompt generation and inference
                try:
                    start_time = time.time()
                    ...
                    # Test inference with auto-prompts
                    pred_mask, num_final_masks = predict_with_auto_prompts(sam2_predictor, image, auto_prompts)
                    ...
                    sample_metrics = metrics.compute_pixel_metrics(pred_mask.astype(np.uint8), gt_mask.astype(np.uint8))
                    
                    print(f"   {img_file}: IoU={sample_metrics['iou']:.3f}, Time={inference_time:.3f}s, Masks={num_final_masks}")
                    success_count += 1
...
def main():
    ...
    parser.add_argument('--checkpoint', type=str, 
                       default='/home/ptp/sam2/new_src/training/training_results/sam2_full_dataset/sam2_small_full_dataset_lr0.0001_20250829_1601/best_step4000_iou0.4302_dice0.5687.torch',
