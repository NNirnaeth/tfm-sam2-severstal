#!/usr/bin/env python3
"""
Evaluate SAM2 model (Small or Large) trained on full Severstal dataset
using the best checkpoint from training.

This script evaluates the fine-tuned SAM2 model on the complete test split
...
def evaluate_sam2_full_dataset(checkpoint_path, results_dir, model_size="large", num_viz_samples=10, 
                              evaluation_mode="auto_prompt", num_gt_points=3):
    """Evaluate SAM2 model (Small or Large) trained on full dataset
    ...
    print(f"\nEvaluating SAM2 {model_size.title()} (Fine-tuned on Full Dataset)...")
    ...
    try:
        print(f"Loading fine-tuned SAM2 {model_size.title()} model...")
        ...
        try:
            ...
            if "'model'" in str(e):
                ...
                print("Fine-tuned SAM2 model loaded successfully!")
            else:
                raise e
    ...
    model_info = {
        'model': f'sam2_{model_size.lower()}',
    ...
    # Save results to CSV
    experiment_name = f"eval_sam2_{model_size.lower()}_full_dataset_{datetime.now().strftime('%Y%m%d_%H%M')}"
    csv_path = metrics_calculator.save_results_to_csv(
        batch_results, experiment_name, model_info, additional_info
    )
    
    # Print comprehensive summary
    metrics_calculator.print_metrics_summary(batch_results, f"SAM2 {model_size.title()} (Fine-tuned)")
...
def test_script_functionality(checkpoint_path, model_size="large", num_test_images=5, 
                             evaluation_mode="auto_prompt", num_gt_points=3):
    ...
    # Test model loading
    try:
        print(f"Testing {model_size} model loading...")
        # Change to sam2base directory for proper config resolution
        original_dir = os.getcwd()
        sam2base_dir = "/home/ptp/sam2/libs/sam2base"
        os.chdir(sam2base_dir)
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate SAM2 (Small or Large) on full Severstal dataset')
    ...
    parser.add_argument('--model_size', type=str, 
                       choices=['small', 'large'],
                       default='large',
                       help='SAM2 model size: small or large')
    ...
    # Test mode
    if args.test_only:
        test_script_functionality(args.checkpoint, model_size=args.model_size, 
                                 evaluation_mode=args.evaluation_mode, num_gt_points=args.num_gt_points)
        return
    ...
    # Run evaluation
    results = evaluate_sam2_full_dataset(
