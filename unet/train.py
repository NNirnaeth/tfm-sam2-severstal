#!/usr/bin/env python3
"""
Train U-Net++ model on full Severstal dataset using modular architecture
...
- experiments/model_unet.py for model creation
...
def load_checkpoint(checkpoint_path, model, optimizer, scheduler, device):
    ...
    start_epoch = checkpoint.get('epoch', 0) + 1  # Next epoch to train
    ...
    return start_epoch, best_val_dice, best_val_iou


def main():
    parser = argparse.ArgumentParser(description='Train UNet/UNet++ on Severstal dataset')
    ...
    parser.add_argument('--start_epoch', type=int, default=0,
                   help='Starting epoch (useful when resuming)')
    ...
    # Create model using modular model creation
    ...
    # Training loop
    if args.resume:
        start_epoch, best_val_dice, best_val_iou = load_checkpoint(
