#!/usr/bin/env python3
"""
Train U-Net++ model with EfficientNet backbones on full Severstal dataset
...
Supports EfficientNet B0-B7 backbones via timm library
...
from model_unet_efficientnet import create_efficientnet_unet_model
from prepare_data_unet import create_data_loaders
...
def main():
    parser = argparse.ArgumentParser(description='Train UNet/UNet++ with EfficientNet on Severstal dataset')
    ...
    parser.add_argument('--save_dir', type=str, default='new_src/training/training_results/efficientnet_models',
    ...
    parser.add_argument('--encoder', type=str, default='efficientnet_b0',
                       ...
                       help='EfficientNet backbone (default: efficientnet_b0)')
    parser.add_argument('--unet_plus_plus', action='store_true',
                       help='Use UNet++ instead of standard UNet')
    ...
    if args.use_wandb:
        wandb.init(
            project="unet-efficientnet-severstal-tfm",
            name=args.run_name or f"unet_{args.encoder}_{'++' if args.unet_plus_plus else ''}_lr{args.lr}_seed{args.seed}",
            config=vars(args)
        )
    ...
    # Create model using EfficientNet
    model = create_efficientnet_unet_model(
        encoder_name=args.encoder,
        unet_plus_plus=args.unet_plus_plus,
        in_channels=3,
        num_classes=1
    ).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        if val_dice > best_val_dice:
            ...
            best_ckpt_path = os.path.join(args.save_dir, f"best_dice_{val_dice:.4f}_{args.encoder}_seed{args.seed}.pth")
            torch.save({
                'epoch': epoch,
        ...
        if val_iou > best_val_iou:
            ...
            best_iou_ckpt_path = os.path.join(args.save_dir, f"best_iou_{val_iou:.4f}_{args.encoder}_seed{args.seed}.pth")
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
        ...
        if (epoch + 1) % 10 == 0:
            ckpt_path = os.path.join(args.save_dir, f"checkpoint_epoch_{epoch+1}_{args.encoder}_seed{args.seed}.pth")
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
    ...
    metrics_csv_path = os.path.join(args.save_dir, f"metrics_{args.encoder}_seed{args.seed}.csv")
